...
def inversion (σ: Perm (Fin n)): Set (Fin n × Fin n) := { s | s.1 < s.2 ∧ σ s.1 > σ s.2 }

instance lt_by_inversion: LT (Perm (Fin n)) := ⟨λ σ π => inversion σ ⊂ inversion π⟩
instance le_by_inversion: LE (Perm (Fin n)) := ⟨λ σ π => inversion σ ⊆ inversion π⟩

instance inversion_po : PartialOrder (Perm (Fin n)) where
...
theorem id_181 {σ π: Perm (Fin n)}: σ ⋖ π ↔
  (Nat.card (inversion σ) + 1 = Nat.card (inversion σ)):= by
  apply Iff.intro
  · intro h
    dsimp [· ⋖ ·] at h
    sorry
