...
def balanced (S : Set ℝ) : Prop :=
  ∀ A ∈ S, ∀ B ∈ S, A ≠ B → (∃ C ∈ S, dist A C = dist B C)

def centre_free (S : Set ℝ) : Prop :=
...
-/
theorem imo_2015_p1 :
  (∀ n ≥ 3, ∃ (S : Finset ℝ), balanced S ∧ S.card = n) ∧
  (∀ n ≥ 3, (∃ (S : Finset ℝ), balanced S ∧ centre_free S ∧ S.card = n) ↔ Odd n) := by sorry
